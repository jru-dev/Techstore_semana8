...
const register = async (req, res) => {
    try {
        ...
        await logAction(result.rows[0].id, 'INSERT', 'usuarios', `Nuevo usuario registrado: ${email}`);
...
const login = async (req, res) => {
    try {
        ...
        if (user.bloqueado) {
            await logAction(user.id, 'LOGIN_BLOQUEADO', 'auth', `Intento de login en cuenta bloqueada: ${email}`);
        ...
        if (!validPassword) {
            ...
            if (newAttempts >= 5) {
                ...
                await logAction(user.id, 'LOGIN_FALLIDO', 'auth', `Usuario bloqueado tras 5 intentos fallidos: ${email}`);
            ...
            await logAction(user.id, 'LOGIN_FALLIDO', 'auth', `Intento fallido ${newAttempts}/5 para: ${email}`);
            return res.status(401).json({ message: 'Credenciales inválidas' });
        }
        ...
        const tempToken = jwt.sign(
            { userId: user.id, email: user.email, requiresMFA: true },
        ...
        if (!globalMfaEnabled) {
            ...
            await logAction(user.id, 'LOGIN_EXITOSO', 'auth', `Login completo exitoso (MFA Global Desactivado): ${user.email}`);
        ...
        if (user.mfa_habilitado) {
            await logAction(user.id, 'LOGIN_PASO1', 'auth', `Credenciales correctas, esperando TOTP: ${email}`);
            return res.json({
        ...
        await logAction(user.id, 'LOGIN_PASO1', 'auth', `Credenciales correctas, código MFA enviado por email: ${email}`);
...
const verifyMFA = async (req, res) => {
    try {
        ...
        if (!isValid) {
            await logAction(user.id, 'MFA_FALLIDO', 'auth', `Código MFA incorrecto para: ${user.email}`);
        ...
        await logAction(user.id, 'LOGIN_EXITOSO', 'auth', `Login completo exitoso: ${user.email}`);
...
const setupMFA = async (req, res) => {
    try {
        ...
        await logAction(userId, 'MFA_SETUP', 'auth', `Inicio de configuración MFA TOTP para usuario ID: ${userId}`);
...
const confirmMFA = async (req, res) => {
    try {
        ...
        await logAction(userId, 'MFA_CONFIRMADO', 'auth', `MFA TOTP activado para usuario ID: ${userId}`);
...
const sendMFACode = async (req, res) => {
    try {
        ...
        await logAction(decoded.userId, 'MFA_REENVIO', 'auth', `Código MFA reenviado por email a: ${userResult.rows[0].email}`);
...
// Obtener estado del MFA global
const getMfaStatus = (req, res) => {
    res.json({ globalMfaEnabled });
};

// Alternar MFA global
const toggleMfa = async (req, res) => {
    try {
        globalMfaEnabled = !globalMfaEnabled;
        await logAction(req.userId, 'MFA_TOGGLE', 'auth', `MFA Global cambiado a: ${globalMfaEnabled}`);
        res.json({ message: `MFA global ${globalMfaEnabled ? 'activado' : 'desactivado'}`, globalMfaEnabled });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Error cambiando estado de MFA' });
    }
};
